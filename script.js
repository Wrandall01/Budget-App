...
function scheduleSave() {
  ...
  saveTimer = setTimeout(() => { saveLocal(); saveCloud(); }, 300);
}
...
function addExpense({dateISO,categoryKey,name,amount}) {
  ...
  if (!(dateY>=2026 && dateY<=2100) || !(dateM>=1 && dateM<=12)) { toast('Date hors plage (2026–2100).'); return; }
...
function monthlyStats() {
  ...
  const totals = rows.reduce((a,r) => {
    a.budget += r.budget; a.spent += r.spent; a.remaining += r.remaining; return a;
...
function renderMonthlySummary() {
  ...
  rows.forEach(r => {
    ...
    const title = el('div', { class:'title' }, [
      el('span', { text:r.name }),
      el('div', { class:'chips' }, [ el('span', { class:'chip', text:'Mensuel' }) ])
    ]);
    ...
    const rDiv = el('div');
    rDiv.innerHTML = `<span${remColor}>${fmtEUR.format(r.remaining)}</span>`;
    ...
    btn.addEventListener('click', (e) => {
      ...
      menu.style.left = '0px'; menu.style.top = '30px';
    });
    kb.appendChild(btn);
    kb.appendChild(menu);
    ...
    detailBtn.addEventListener('click', () => {
      ...
      detailBtn.textContent = vis ? 'Afficher le détail' : 'Masquer le détail';
      menu.style.display = 'none';
    });
    editBtn.addEventListener('click', () => { menu.style.display='none'; openEditCategoryModal(r.id); });
    delBtn.addEventListener('click', () => { menu.style.display='none'; openDeleteCategoryModal(r.id); });
  });
}
...
function buildExpenseDetailRow({date,name,amount,onEdit,onDelete}) {
  ...
  expBtn.addEventListener('click', (ev) => {
    ...
    expMenu.style.left = '0px'; expMenu.style.top = '30px';
  });
  expEdit.addEventListener('click', () => { expMenu.style.display='none'; onEdit?.(); });
  expDel .addEventListener('click', () => { expMenu.style.display='none'; onDelete?.(); });
...
function openEditCategoryModal(catId) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    cat.name = name; cat.budget = budget;
    scheduleSave(); close(); refreshAll(); toast('Catégorie modifiée.');
  });
...
function openDeleteCategoryModal(catId) {
  ...
  overlay.querySelector('#confirmDel').addEventListener('click', () => {
    ...
    if (store.months[ym]?.monthlyCategories?.[catId]) {
      ...
      scheduleSave(); refreshAll(); toast('Catégorie supprimée.');
    }
    close();
  });
}
...
function openEditExpenseModal(catId, expIndex) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    scheduleSave(); close(); refreshAll(); toast('Dépense modifiée.');
  });
...
function openDeleteExpenseModal(catId, expIndex) {
  ...
  overlay.querySelector('#confirmExpDel').addEventListener('click', () => {
    ...
    scheduleSave(); refreshAll(); toast('Dépense supprimée.');
    close();
  });
}
...
function openEditAnnualCategoryModal(catId) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!name || !(budget>=0)) { toast('Veuillez renseigner correctement.'); return; }
    cat.name = name; cat.budget = budget;
    scheduleSave(); close(); refreshAll(); toast('Catégorie annuelle modifiée (impacte toute l’année).');
  });
...
function openDeleteAnnualCategoryModal(catId) {
  ...
  overlay.querySelector('#confirmAnnualDel').addEventListener('click', () => {
    ...
    if (store.years[y]?.annualCategories?.[catId]) {
      ...
      scheduleSave(); refreshAll(); toast('Catégorie annuelle supprimée (toute l’année).');
    }
    close();
  });
}
...
function openEditAnnualExpenseModal(catId, expIndex) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!name || !(amount>=0)) { toast('Veuillez renseigner correctement.'); return; }
    exp.name = name; exp.amount = amount;
    scheduleSave(); close(); refreshAll(); toast('Dépense annuelle modifiée (impacte toute l’année).');
  });
...
function openDeleteAnnualExpenseModal(catId, expIndex) {
  ...
  overlay.querySelector('#confirmAnnualExpDel').addEventListener('click', () => {
    cat.expenses.splice(expIndex,1);
    scheduleSave(); refreshAll(); toast('Dépense annuelle supprimée (toute l’année).');
    close();
  });
}
...
function renderAnnualSummary() {
  ...
  rows.forEach(r => {
    ...
    const title = el('div', { class:'title' }, [
      el('span', { text:r.name }),
      el('div', { class:'chips' }, [ el('span', { class:'chip', text:'Annuel' }) ])
    ]);
    ...
    editBtn.addEventListener('click', () => { menu.style.display='none'; openEditAnnualCategoryModal(r.id); });
    delBtn.addEventListener('click', () => { menu.style.display='none'; openDeleteAnnualCategoryModal(r.id); });
  });
}
...
document.querySelector('#addExpenseBtn').addEventListener('click', () => {
  ...
  if (!dateInput._bound) {
    dateInput.addEventListener('change', () => populateExpenseCategorySelect(dateInput.value));
...
document.querySelectorAll('[data-close]').forEach(btn =>
  btn.addEventListener('click', () => { const sel = btn.getAttribute('data-close'); const m = document.querySelector(sel); if (m) m.classList.add('hidden'); })
);
...
document.querySelector('#categoryForm').addEventListener('submit', (e) => {
  ...
  if (!name || !(budget>=0)) { toast('Veuillez renseigner correctement la catégorie.'); return; }
...
function populateExpenseCategorySelect(dateISO) {
  ...
  if (!dateISO) {
    ...
    info.value = ''; info.textContent = 'Choisissez une date';
    sel.appendChild(info); sel.setAttribute('disabled','disabled'); return;
  }
  ...
  if (monthlyCats.length === 0 && annualCats.length === 0) {
    const info = document.createElement('option');
    info.value = ''; info.textContent = `Aucune catégorie pour ${monthNames[m-1]} ${y}.`;
    sel.appendChild(info); return;
  }
  ...
  if (monthlyCats.length > 0) {
    ...
    monthlyCats.forEach(c => {
      ...
      opt.value = `m:${ymDate}:${c.id}`; opt.textContent = c.name;
  ...
  if (annualCats.length > 0) {
    ...
    annualCats.forEach(c => {
      const opt = document.createElement('option');
      opt.value = `a:${y}:${c.id}`; opt.textContent = c.name;
...
document.querySelector('#expenseForm').addEventListener('submit', (e) => {
  ...
  if (!dateISO || !categoryKey || !name || !(amount>=0)) { toast('Veuillez renseigner correctement la dépense.'); return; }
...
function updateAuthStatus() {
  ...
  authStatus.textContent = u ? (u.email || 'Connecté') : 'Non connecté';
}
...
// ✅ Décision d’ouverture/fermeture de modale basée UNIQUEMENT sur onAuthStateChanged
if (USE_FIREBASE && window._fb && window._fb.onAuthStateChanged && window._fb.auth) {
  window._fb.onAuthStateChanged(window._fb.auth, async (user) => {
    updateAuthStatus();
    if (user) {
      // session restaurée : fermer la modale si visible et binder la data
      authModal?.classList.add('hidden');
      await bindStore();
    } else {
      // pas de session : ouvrir la modale
      openAuth();
    }
  });
}

// ❌ Retiré : ne pas ouvrir la modale au chargement sur base de currentUser (nul avant restauration)
// if (USE_FIREBASE) {
//   if (!window._fb || !window._fb.app) {
//     console.warn('Firebase non configuré. Utilisation du localStorage.');
//   } else {
//     if (!window._fb.auth?.currentUser) { openAuth(); } // <-- ouvrait trop tôt
//   }
//}

// Handlers de connexion/inscription
authForm?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    toast('Connexion impossible.'); console.warn(err);
  }
});
...
authCreate?.addEventListener('click', async () => {
  ...
  try {
    ...
    toast('Création impossible.'); console.warn(err);
  }
});
