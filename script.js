...
/* ====== Configuration ====== */
const USE_FIREBASE = true; // passe à false pour rester 100% localStorage
...
function addExpense({ dateISO, categoryKey, name, amount }){
  ...
  if(!(dateY>=2026 && dateY<=2100) || !(dateM>=1 && dateM<=12)){
    toast('Date hors plage (2026–2100).'); return;
  }
  ensurePeriod(dateY, dateM);

  const parts = categoryKey.split(':'); // m:YYYY-MM:<id> ou a:YYYY:<id>
  const scope = parts[0];

  if(scope === 'm'){
    const srcYM = parts[1]; const id = parts[2];
    ...
    const srcY = parseInt(parts[1],10); const id = parts[2];
...
function monthlyStats(){
  ...
  const totals = rows.reduce((a,r)=>{ a.budget+=r.budget; a.spent+=r.spent; a.remaining+=r.remaining; return a; }, {budget:0,spent:0,remaining:0});
  return { rows, totals };
}
...
function renderMonthlySummary(){
  ...
  rows.forEach(r=>{
    ...
    kb.appendChild(btn); kb.appendChild(menu);
    ...
    editBtn.addEventListener('click',()=>{
      menu.style.display='none'; openEditCategoryModal(r.id);
    });
    delBtn.addEventListener('click',()=>{
      menu.style.display='none'; openDeleteCategoryModal(r.id);
    });
  });
}
...
function openEditCategoryModal(catId){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    cat.budget = budget;
    scheduleSave();
    close(); refreshAll(); toast('Catégorie modifiée.');
  });
...
function openEditExpenseModal(catId, expIndex){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    exp.amount = amount;
    scheduleSave();
    close(); refreshAll(); toast('Dépense modifiée.');
  });
...
function openEditAnnualCategoryModal(catId){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    cat.budget = budget;
    scheduleSave();
    close(); refreshAll(); toast('Catégorie annuelle modifiée (impacte toute l’année).');
  });
...
function openEditAnnualExpenseModal(catId, expIndex){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    exp.amount = amount;
    scheduleSave();
    close(); refreshAll(); toast('Dépense annuelle modifiée (impacte toute l’année).');
  });
...
function renderAnnualSummary(){
  ...
  rows.forEach(r=>{
    ...
    if(r.expenses.length===0){
      details.appendChild(el('div',{class:'details-row'},[
        el('div',{text:'—'}), el('div',{text:'Aucune dépense annuelle'}), el('div',{text:fmtEUR.format(0)}), el('div')
      ]));
    }else{
      r.expenses.forEach((e,idx)=>{
        details.appendChild(buildExpenseDetailRow({
    ...
    editBtn.addEventListener('click',()=>{
      menu.style.display='none'; openEditAnnualCategoryModal(r.id);
    });
    delBtn.addEventListener('click',()=>{
      menu.style.display='none'; openDeleteAnnualCategoryModal(r.id);
    });
  });
}
...
document.querySelector('#expenseForm').addEventListener('submit',(e)=>{
  ...
  if(!dateISO || !categoryKey || !name || !(amount>=0)){
    toast('Veuillez renseigner correctement la dépense.'); return;
  }
...
/* ====== Firebase Auth & binding (persistante) ====== */
...
authStatus.title = 'Connexion / Déconnexion';
authStatus.addEventListener('click', ()=>{
  const u = window._fb?.auth?.currentUser;
  if(!u){
    authModal.classList.remove('hidden');
  }else{
    if(confirm(`Se déconnecter de ${u.email} ?`)){
...
async function bindStore(){
  if(!window._fb?.db || !window._fb?.auth?.currentUser) return;
...
if(window._fb?.onAuthStateChanged && window._fb?.auth){
  window._fb.onAuthStateChanged(window._fb.auth, async (user)=>{
    updateAuthStatus();
    if(user){ await bindStore(); }
    // Pas d’ouverture de modal automatique quand user==null :
    // on laisse l’utilisateur cliquer sur le chip si besoin.
  });
}
